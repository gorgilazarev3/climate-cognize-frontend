...
const ClimateModelsInput = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedTask === ClimateModelTasks.CLIMATE_DETECTION) {
      ClimateCognizeService.sendToDetectModel(inputData, selectedModel).then((value) => {
        if (value.status === 200) {
      ...
      ClimateCognizeService.sendToSentimentModel(inputData, selectedModel).then((value) => {
        if (value.status === 200) {
          handleResponse(value.data);
        }
      });
      ...
      ClimateCognizeService.sendToSpecificityModel(inputData, selectedModel).then((value) => {
        if (value.status === 200) {
          handleResponse(value.data);
        }
      });
      ...
      ClimateCognizeService.sendToCommitmentsModel(inputData, selectedModel).then((value) => {
        if (value.status === 200) {
          handleResponse(value.data);
        }
      });
      ...
      ClimateCognizeService.sendToTCFDModel(inputData, selectedModel).then((value) => {
        if (value.status === 200) {
          handleResponse(value.data);
        }
      });
