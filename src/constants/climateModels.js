export const ClimateModels = {
    CLIMATEBERT_CLIMATE_DETECTOR: "climatebert-climate-detector",
    CLIMATEBERT_CLIMATE_SENTIMENT: "climatebert-climate-sentiment",
    CLIMATEBERT_CLIMATE_SPECIFICITY: "climatebert-climate-specificity",
    CLIMATEBERT_CLIMATE_COMMITMENTS_AND_ACTIONS: "climatebert-climate-commitments-actions",
    CLIMATEBERT_CLIMATE_TCFD:"climatebert-climate-tcfd",
    PERSONAL_MODEL_CLIMATE_DETECTION: "our-model-climate-detection",
};